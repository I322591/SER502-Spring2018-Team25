...
public class ExecuteROCH {
	...
	public static void relationalOperation(List<String> tokenList) {
		try {
			...
			case "GE":
				updateValue(
						Arrays.asList(tokenList.get(1), Boolean.toString(operand1 >= operand2)));
				break;
			case "LT":
				updateValue(Arrays.asList(tokenList.get(1), Boolean.toString(operand1 < operand2)));
				break;
	...
	public static void logicalOperation(List<String> tokenList) {
		try {
			switch (tokenList.get(0)) {
			case "AND":
				Arrays.asList(tokenList.get(1),
						Boolean.toString(Boolean.valueOf(getValue(tokenList.get(1)))
								&& Boolean.valueOf(getValue(tokenList.get(2)))));
				break;
			case "OR":
				Arrays.asList(tokenList.get(1),
						Boolean.toString(Boolean.valueOf(getValue(tokenList.get(1)))
	...
	public static void untilCondition(List<String> tokenList) {
	...
	public static void untilEndCondition(List<String> tokenList) {
		RunROCH.LineNum = Integer.parseInt(getValue("UNTIL")) - 1;
	}

	public static void checkCondition(List<String> tokenList) {
		if (getValue(tokenList.get(1)).compareToIgnoreCase("true") == 0)
			insert("CHECK", "NULL");
		else
	...
	public static void otherWiseCondition(List<String> tokenList) {
		if (tokenList.size() == 1)
